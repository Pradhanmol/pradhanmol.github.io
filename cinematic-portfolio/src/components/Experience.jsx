        ...
        >
          {experiences.map((exp, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="flex-shrink-0 w-[400px] space-y-6 relative"
            >
              {/* Vertical line */}
              <div className="absolute left-0 top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-white/20 to-transparent" />
              
              <div className="pl-8">
                <p className="font-serif text-6xl font-light text-gray-800 mb-2">
                  {exp.year}
                </p>
                
                <h3 className="font-sans text-2xl font-light mb-2">
                  {exp.company}
                </h3>
                
                <p className="font-sans text-gray-500 text-sm tracking-wider uppercase mb-1">
                  {exp.role}
                </p>
                
                <p className="font-sans text-gray-600 text-xs tracking-wider uppercase mb-6">
                  {exp.location}
                </p>
                
                <ul className="space-y-2">
                  {exp.highlights.map((highlight, i) => (
                    <li key={i} className="font-sans text-gray-400 text-sm">
                      — {highlight}
                    </li>
                  ))}
                </ul>
